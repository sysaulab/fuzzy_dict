...
While these approaches are effective, they often require significant memory overhead or complex data structures. We propose a simpler alternative: a **character-presence bitmask filter** that uses bitwise operations to perform O(1) containment verification, rejecting candidates that lack the characters necessary to form the query.

## 2. The Bitmask Filter Algorithm

### 2.1 Core Idea

The filter operates on the observation that if a candidate string does not contain *all* the characters present in the query, it cannot possibly match the query under any reasonable similarity metric. This necessary condition is both conservative (no false negatives) and computationally trivial to check.

### 2.2 Bitmask Representation

Each string is represented by a fixed-width bitmask where each bit corresponds to the presence of a specific character or character class. For a 64-bit mask:

- **Bit 0**: Reserved for "unknown" characters (any character not explicitly mapped)
- **Bits 1–63**: Assigned to character classes (e.g., 'a', 'b', ..., 'z', digits, symbols, accented variants)

The assignment of bits to characters is flexible and can be tailored to the application's alphabet. For alphabetic scripts, each line in an alphabet definition file assigns a bit to a group of characters (e.g., `aAáÀâÄ` all share the same bit). This grouping enables:
...
### 2.3 Containment Verification

Given a query \( q \) with mask \( M(q) \), a candidate string \( t \) with mask \( M(t) \) can be rejected if:

\[
(M(q) \ \& \ M(t)) \neq M(q)
\]

This bitwise AND operation checks whether all bits set in the query mask are also set in the candidate mask. If any query character is absent from the candidate, the condition fails and the candidate is immediately rejected.

**Key property**: This filter is **conservative**—it never produces false negatives. Any candidate that passes the filter *may* match the query, but any candidate that fails *definitely* does not match.

### 2.4 Bucketing Optimization

To avoid scanning all candidates, the dictionary is partitioned into **buckets** keyed by their full masks. During search:

1. Compute the query mask \( M(q) \).
2. Retrieve the bucket for the exact mask.
3. If no satisfactory match is found, also check buckets whose masks differ by 1 or 2 bit flips (handling insertion/deletion of character classes).
4. Score only candidates from these buckets using the full similarity metric (e.g., Jaro-Winkler).

This bucketing strategy reduces the candidate set from the entire dictionary to only those with compatible character sets—typically 5–20% of the total.
...
| Bucket lookup | O(1) |
| Containment check | O(1) (single bitwise AND) |
| Bucket expansion (1–2 bit flips) | O(B²) where B = number of effective bits |

The filter adds minimal overhead: a 64-bit integer per dictionary entry and a single bitwise AND per candidate check.

## 3. Implementation Considerations

### 3.1 Alphabet Definition

The alphabet is defined in a simple line-based format. Each line assigns a bit to all characters listed on that line:
...
- **Case insensitivity**: Both cases on the same line
- **Accent folding**: Accented variants grouped with base letters
- **Multi-script support**: Latin, Cyrillic, Greek, and Arabic alphabets can be merged line-wise

The merging strategy aligns lines across files: line 0 from all files shares bit 1, line 1 shares bit 2, etc. This keeps the total bit count equal to the maximum number of lines across any single alphabet file.
...
### 3.3 Zero-Copy Candidate Collection

To minimize allocations during search, candidates are collected as `&str` references from the buckets. Only after scoring and filtering are results converted to owned strings. This zero-copy approach reduces GC pressure and improves throughput.

### 3.4 Length-Based Prioritization

Candidates from expanded buckets are sorted by absolute length difference from the query before scoring. This prioritizes words of similar length, which are more likely to be relevant, and enables early termination once a quota of results is reached.
...
- **80–95% rejection rate** for typical queries
- **O(1) rejection** with a single bitwise AND operation

Comparative benchmarks from similar systems show:

| Library | Time per search (10K items) |
|---------|----------------------------|
| fuzzysort (prepared) | 179 µs |
| FuzzyIndex (with bitmask filter) | ~232 µs |
| uFuzzy (regex-based filter) | 621 µs |

The bitmask filter closes the performance gap with specialized fuzzy finders while maintaining a simpler implementation.
...
For applications requiring higher precision, the filter can be combined with:
- N-gram inverted indices for order-sensitive filtering
- Length filtering as a secondary pre-filter
...
The Bitap algorithm (also known as Shift-Or or Baeza-Yates-Gonnet) uses bit-parallelism to perform approximate string matching. Unlike our filter, Bitap computes edit distances directly using bit vectors and is typically used for online pattern matching in text, not for dictionary lookup.
...
- Romanization (converting CJK to pinyin/romaji before filtering)
- N-gram tokenization of character sequences
...
### 6.4 SIMD Acceleration

The bitwise AND operation is already minimal, but SIMD instructions could batch multiple containment checks for vectorized filtering of large candidate sets.

## 7. Conclusion

We have presented a character-presence bitmask filter for high-performance fuzzy string matching. The filter uses a 64-bit mask to represent the character set of each string and performs O(1) containment verification with a single bitwise AND operation. It is conservative (no false negatives), memory-efficient (8 bytes per entry), and can reject 80–95% of candidates before any expensive similarity computation.
